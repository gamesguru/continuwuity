...
Sync Tokens generated by older versions are not guaranteed to remain valid after
upgrading across those storage and receipt changes. Clients should perform a
fresh sync without the old token; operators should ask users to clear stale sync
state or restart affected clients if incremental sync fails after upgrade.
