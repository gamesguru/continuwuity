...
Docker bridge networks uses a non-performant resolver to intercept and respond to container hostnames, and **this should also be avoided**. Instead, use the `dns:` key in your `docker-compose.yml` or mount a custom `/etc/resolv.conf` file into the container to bypass Docker's internal resolver.

It is recommended to run a dedicated resolver container (like Unbound) for Continuwuity. Our default `docker-compose.yml` includes an Unbound sidecar configured this way.
...
If Continuwuity is running in an environment where no system nameservers can be found (common in `scratch` Docker images without a sidecar), it will use its internal fallback mechanism.

By default, it uses **Quad9 (9.9.9.9)**, a privacy-respecting, non-logging resolver. You can override this in your configuration:

```toml title="continuwuity.toml"
[global]
# List of fallback DNS servers
dns_fallbacks = ["9.9.9.9", "1.1.1.1"]
```

**Note:** Fallbacks are only used if no system-level nameservers are available.
