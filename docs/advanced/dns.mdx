...
Docker bridge networks use a non-performant resolver to intercept and respond to container hostnames, and **this should also be avoided**. Instead, use the `dns:` key in your `docker-compose.yml` or mount a custom `/etc/resolv.conf` file into the container to bypass Docker's internal resolver.
