...
Docker's default DNS resolver is known to [cause timeout issues](../troubleshooting#dns-issues) for Matrix federation. To solve this, our recommended deployment method uses a dedicated **Unbound sidecar** container.
...
Note: In this quick run mode, Continuwuity will use its built-in **DNS Fallbacks (Quad9)** to ensure federation works even without a sidecar.
:::
