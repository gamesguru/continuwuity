...
Docker's default DNS resolver is known to [cause timeout issues](../troubleshooting#dns-issues) for Matrix federation. The default `docker-compose.yml` includes a dedicated **Unbound sidecar** container, while some proxy-specific examples still use custom resolver mounts.

Consult the [**DNS tuning guide (recommended)**](../advanced/dns.mdx) for more details.
:::
...
For production deployment, we recommend using [Docker Compose](#docker-compose) which includes a dedicated **Unbound sidecar** for high-performance DNS.

Note: In this quick run mode, Continuwuity will usually use whatever nameservers Docker provides inside the container (often Docker's internal resolver). Its built-in **DNS Fallbacks (Quad9)** are only used when the container has no system nameservers configured. If you want to avoid Docker's internal DNS for testing federation, explicitly override the container DNS settings or use the Docker Compose setup with the Unbound sidecar.
:::
