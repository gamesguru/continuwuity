...
You can allow this traffic on the Docker bridge interfaces. Since UFW does not support glob matching (`br-+`) for interface names, you must specify the exact interface (e.g. `docker0` or a specific `br-<id>`). Find the interface name by running `ip link` or `ifconfig`, then add the rule:

```bash
# Replace `docker0` with your specific docker bridge interface
sudo ufw allow in on docker0 comment "Allow traffic from custom Docker networks for Complement"
```
