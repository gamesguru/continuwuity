...
impl Drop for TransactionContext {
	fn drop(&mut self) {
		...
		let finish_closures = std::mem::take(&mut self.on_finish);
		for closure in finish_closures {
			if let Err(e) = std::panic::catch_unwind(std::panic::AssertUnwindSafe(closure)) {
				let msg = e
					.downcast_ref::<&'static str>()
					.copied()
...
/// Adds a closure to execute when the transaction finishes (commits or rolls
/// back). Returns true if closure added to a txn, false if no txn active.
pub fn push_on_finish<F>(f: F) -> bool
where
	F: FnOnce() + Send + 'static,
{
	TRANSACTION_BATCH
		.try_with(|txn| {
			let mut txn_guard = txn.lock().expect("Transaction batch mutex poisoned");
			txn_guard.on_finish.push(Box::new(f));
		})
		.is_ok()
}
