...
use bytes::Bytes;
use conduwuit::{
	Err, Error, Result, debug, debug::INFO_SPAN_LEVEL, debug_error, err, implement,
...
fn handle_error(
	...
	if e.is_connect() {
		e = e.without_url();
		tracing::warn!(target = "federation_debug", %dest, %method, %url, "Federation connection failed: {e:?}");
