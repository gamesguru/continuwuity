...
use conduwuit::{debug, debug_error, debug_warn, error, info, result::FlatOk, trace, warn};
...
impl super::Service {
	/// Returns all verify keys required to verify the incoming events.
	...
	/// Acquires all the verify keys required in the provided batch, using
	/// notaries and direct-fetch where applicable.
		...
		K: Iterator<Item = &'a ServerSigningKeyId> + Send + Clone,
	{
		...
		if notary_first_always || notary_first_on_join {
			...
			warn!(
				"missing {missing_keys} keys for {missing_servers} servers from all notaries \
				 first"
			);
		}
		...
		if !notary_first_always && !notary_first_on_join {
			...
			debug_warn!(
				"still missing {missing_keys} keys for {missing_servers} servers from all \
				 notaries."
			);
		}
	...
	/// Filters the batch to return only keys which we do not already have
	/// locally.
	...
	/// Fetches verification keys from the origin servers directly, returning
	/// any keys that could not be fetched.
	...
	/// Fetches verification keys from an origin directly, returning any keys
	/// that were fetched.
	...
	/// Fetches signing keys for the given batch from the configured notary
	/// servers, returning any keys that could not be fetched.
	...
	/// Contacts a notary and fetches signing keys for the given batch, removing
	/// keys from the missing batch when they are found.
