...
impl super::Service {
		...
		let required = match required_keys(object, &room_version_rules.signatures) {
			...
			| Err(e) => {
				return Err!(BadServerResponse(
					"Failed to determine keys required to verify event: {e}"
				));
			},
		};
	...
	/// Fetches all verify keys for the given batch. Any keys which could not be
	/// fetched are elided from the resulting map.
	...
	/// Gets verify keys for the given server name and a set of key IDs. Any
	/// keys that cannot be found are elided from the resulting set.
	...
	/// Fetches a single verify key via origin and notaries.
	...
	/// Fetches verify keys from notary servers, adding them to the cache, and
	/// returning the requested key if found.
	...
	/// Fetches verify keys from the origin server, adding them to the cache,
	/// and returning the requested key if found.
