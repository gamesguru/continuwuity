	...
	T: DeviceStore,
{
	...
	pub async fn get_client(&self, client_id: &str) -> Result<Option<OidcClient>> {
		if (self.clientid_oidcclient.exists(client_id).await).is_err() {
			return Ok(None);
		}
		let client = self
			.clientid_oidcclient
	...
	T: DeviceStore,
{
	async fn issue(&mut self, grant: Grant) -> Result<IssuedToken, ()> {
		...
		let device_id =
			deviceid_from_scope(&grant.scope).expect("valid device id from the grant's scope");
		...
		let until = Utc::now()
			.checked_add_signed(Duration::milliseconds(self.token_ttl))
			.expect("valid offset");
		let refresh_until = Utc::now()
			.checked_add_signed(Duration::milliseconds(self.refresh_ttl))
			.expect("valid offset");
		...
		if !self.devices.exists(&user_id, &device_id).await {
			let oidc_device = OidcDevice {
				...
				until: until.timestamp_millis().try_into().unwrap_or(0),
			};
	...
	async fn refresh(&mut self, refresh: &str, _grant: Grant) -> Result<RefreshedToken, ()> {
		...
		if expires_at.try_into().unwrap_or(0) < millis_since_unix_epoch() {
	...
	async fn recover_token<'a>(&'a mut self, token: &str) -> Result<Option<Grant>, ()> {
		...
		let until = DateTime::from_timestamp_millis(device.until.try_into().unwrap_or(i64::MAX))
...
fn deviceid_from_scope(scope: &Scope) -> Result<OwnedDeviceId> {
	...
	Ok(OwnedDeviceId::from(device_id))
}
